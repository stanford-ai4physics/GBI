...
@click.option('--use-trained-weight/--not-use-trained-weight', default=False, show_default=True,
...
def compute_semi_weakly_mu_alpha_likelihood_landscape(**kwargs):
    ...
    def get_arr(name:str, expr:str):
        return np.array(list_of_dict_to_dict_of_list(list(ParamParser.parse_param_str(f'{name}={expr}')))[name])
    ...
    if mixed_signal:
        ...
        llr_2 = llr_2_model.predict(x).flatten()
        llr_3 = llr_3_model.predict(x).flatten()
    else:
        llr_model = model_trainer.model_loader.llr_model
        llr_2 = llr_model.predict(x).flatten()
        llr_3 = None
...
def get_fisher_matrix(model, x: np.ndarray, parameters: List[str], batchsize:int =1024):
    ...
    for index, weight in enumerate(model.trainable_weights):
        ...
        derivatives[name] = param_transforms[name].get_derivative(value)
...
def compute_fisher_information(**kwargs):
    ...
    if use_trained_weight:
        ...
        weights = {
            'm1': param_transforms['m1'].inverse(mass_point[0]),
            'm2': param_transforms['m2'].inverse(mass_point[1]),
            'mu': param_transforms['m2'].inverse(kwargs['mu'])
        }
        if mixed_signal:
            weights['alpha'] = param_transforms['alpha'].inverse(kwargs['alpha'])
    ...
    if parameters is not None:
        parameters = split_str(parameters, sep=',', remove_empty=True)
    else:
        parameters = list(weights)
        
    fisher_matrix = get_fisher_matrix(ws_model, x, parameters=parameters, batchsize=fisher_batchsize)
